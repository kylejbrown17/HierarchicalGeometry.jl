...
get_wireframe_mesh(n) = GeometryBasics.Mesh(collect(coordinates(n)),collect(faces(n)))
function get_wireframe_mesh(n::Cylinder)
    coords = collect(coordinates(n))[1:end-2]
    n = length(coords)/2
    f = [
...
function overapproximate_cylinder(points_and_radii,ϵ=0.0)
    ...
    # get z values
    return Cylinder(
        Point(b.center[1],b.center[2],zlo),
